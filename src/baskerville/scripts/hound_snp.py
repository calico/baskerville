...
def main():
    ...
    sad_out = initialize_output_h5(
        options.out_dir, options.sad_stats, snps, targets_length, targets_strand_df
    )
...
def initialize_output_h5(out_dir, sad_stats, snps, targets_length, targets_df):
    ...
    sad_out = h5py.File("%s/scores.h5" % out_dir, "w")
    ...
    for sad_stat in sad_stats:
        if sad_stat in ["REF", "ALT"]:
            sad_out.create_dataset(
                sad_stat, shape=(num_snps, targets_length, num_targets), dtype="float16"
            )
        else:
            sad_out.create_dataset(
                sad_stat, shape=(num_snps, num_targets), dtype="float16"
            )
...
def write_snp_len(ref_preds, alt_preds, sad_out, si, sad_stats):
    ...
    # compare reference to alternative via mean log division
    if "SADR" in sad_stats:
        sar = np.log2(alt_preds_sum + pseudocounts) - np.log2(
            ref_preds_sum + pseudocounts
        )
        sad_out["SADR"][si] = sar.astype("float16")

    # compare reference to alternative via max subtraction
    if "SAXR" in sad_stats:
        sar_vec = np.log2(alt_preds + pseudocounts) - np.log2(ref_preds + pseudocounts)
